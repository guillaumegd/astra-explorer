...
test('focused bodies follow the GPU column-major rotation and wave displacement', () => {
  ...
  const wave = Math.sin(2 * 2.4) * Math.exp(-4 * 0.32) * 2;
  assert.ok(Math.abs(out.y - base.y - wave * 1.4) < 1e-10);
  assert.ok(Math.abs(out.x - base.x - (base.x / 2) * wave * 0.6) < 1e-10);
});
...
test('system members retain their separation under rotation and gravitational waves', () => {
  ...
  const waves = [new THREE.Vector4(1, 2, 1.5, 0)];
  for (const time of [0, 0.2, 1, 3, 10]) {
    const star = particlePosition(positions, seeds, 0, time, time, waves, new THREE.Vector3(), offsets);
    const planet = particlePosition(positions, seeds, 1, time, time, waves, new THREE.Vector3(), offsets);
