...
const families: {
  ...
  {
    kind: 'Monde océanique',
    type: 4,
    range: [0.0025, 0.009],
    colors: ['#398eac', '#3572bc'],
  },
  {
    kind: 'Planète désertique',
    type: 5,
    range: [0.0015, 0.006],
    colors: ['#d38c5b', '#cbab78'],
  },
  {
    kind: 'Monde volcanique',
    type: 6,
    range: [0.0015, 0.005],
...
export function describeBody(id: number): BodyIdentity {
  let state = (id + 1) >>> 0;
  const random = () => {
...
export function createBodyLOD(parent: THREE.Group) {
  ...
  return {
    update(
      ...
      const active = candidates
        .filter((c) => surfaceVisibility(c.pixels, c.distanceInRadii ?? 0) > 0)
      ...
      for (const candidate of active) {
        ...
        const isNew = !entry;
        if (!entry) {
          if (entries.size >= MAX_DETAILED_BODIES) {
